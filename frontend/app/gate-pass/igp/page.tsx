...
const DEFAULT_ITEMS: IGPItemRow[] = [
  {
    id: "item-1",
    srlNo: 1,
    code: "80535361",
...
function IgpContent() {
  ...
  const [passType, setPassType] = useState<"NORMAL" | "RETURNABLE">("NORMAL");
  ...
  const [instructionText, setInstructionText] = useState(
    "Please receive the following goods brought in by :"
  );
  ...
  const [destination, setDestination] = useState("Yarn Store");
  ...
  // Items
  const [items, setItems] = useState<IGPItemRow[]>(DEFAULT_ITEMS);
  ...
  const [footerDateText, setFooterDateText] = useState(() => {
    const d = new Date();
  ...
  const handleItemChange = (index: number, field: keyof IGPItemRow, value: string) => {
    const updated = [...items];
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (gp) => {
      ...
      setSuccessMsg(`Inward Gate Pass ${gp.gate_pass_number} saved successfully!`);
  ...
  return (
    <div className="flex flex-col gap-5 py-2 animate-fade-in print:p-0 print:m-0">
      ...
      <div className="flex items-center justify-between gap-4 print:hidden bg-white px-5 py-3 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-base font-bold text-navy-900 tracking-tight">Inward Gate Pass (IGP)</h1>
          <p className="text-xs text-slate-500">Al Habib Knitwear — Inward Material Receipts</p>
        </div>
        ...
        <div className="mt-2">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Button
              ...
              className="text-xs h-7 print:hidden font-semibold border-emerald-700 text-emerald-900 hover:bg-emerald-50"
            >
              + Add Row
            </Button>
          </div>
