...
function IgpContent() {
  ...
  const [quantity, setQuantity] = useState(""); // Yarn Received
  ...
  // Fetch IGPs
  const { data: igpData, isLoading } = useQuery({
    queryKey: ["gate-passes", "list", "igp", search, statusFilter, dateFilter],
    queryFn: () =>
      listGatePasses({
        type: "igp",
        q: search || undefined,
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (gp) => {
      ...
      setSuccessMsg(`Inward Gate Pass ${gp.gate_pass_number} created successfully!`);
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (gp) => {
      queryClient.invalidateQueries({ queryKey: ["gate-passes"] });
      setSuccessMsg(`Inward Gate Pass ${gp.gate_pass_number} updated successfully!`);
      setTimeout(() => setSuccessMsg(""), 4000);
      resetForm();
      setShowForm(false);
    },
  ...
  const validateForm = () => {
    ...
    if (!quantity || parseFloat(quantity) <= 0) errors.quantity = "Yarn Received quantity must be greater than 0.";
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const payload = {
      ...
      yarn_return: yarnReturn ? parseFloat(yarnReturn) : null,
    ...
    <div className="flex flex-col gap-6 py-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold text-navy-900 tracking-tight">Inward Gate Pass (IGP)</h1>
          <p className="text-sm text-slate-500">Manage inward material receipts and warehouse routing.</p>
        </div>
        {isWritable && (
          <Button
            onClick={() => {
          ...
          >
            {showForm ? "Cancel / Close" : "New Inward Gate Pass"}
          </Button>
        )}
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="p-5 border-b border-slate-200 bg-slate-50/50">
              <h2 className="text-sm font-semibold text-navy-900 uppercase tracking-wider">
                {editingGp ? `Edit Gate Pass: ${editingGp.gate_pass_number}` : "Create Inward Gate Pass Form"}
              </h2>
            </div>
            <CardContent className="p-6 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <label className="text-sm font-medium text-slate-600">
                    ...
                    <Input
                      disabled
                      value={editingGp ? editingGp.gate_pass_number : "IGP-XXXXX (Auto-generated)"}
              ...
              <div className="flex flex-col gap-4 border-t border-slate-100 pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <label className="text-sm font-medium text-slate-600">
                    Yarn Received Quantity <span className="text-red-500">*</span>
                    <Input
                      type="number"
                      step="0.01"
                      value={quantity}
                  ...
                  <label className="text-sm font-medium text-slate-600">
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={yarnReturn}
                      onChange={(e) => setYarnReturn(e.target.value)}
              ...
              <div className="flex items-center gap-3 border-t border-slate-100 pt-6">
                <Button type="submit" disabled={createMutation.isPending || updateMutation.isPending}>
                  {createMutation.isPending || updateMutation.isPending ? "Saving..." : "Save IGP"}
                </Button>
                <Button
                  type="button"
                  variant="outline"
      ...
      <Card className="border border-slate-200">
        <CardContent className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <Input
            placeholder="Search IGP No., material, count..."
            value={search}
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        {isLoading ? (
          <p className="text-sm text-slate-400 py-4 text-center">Loading IGP records...</p>
        ) : !igpData || igpData.data.length === 0 ? (
          <div className="py-8 text-center text-slate-400 text-sm">No Inward Gate Pass records found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-700">
              <thead className="bg-slate-50 text-xs font-semibold text-slate-500 uppercase border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3">IGP No.</th>
          ...
          <Card className="w-full max-w-xl border border-slate-200 bg-white shadow-2xl">
            ...
            <CardContent className="p-6 flex flex-col gap-4 text-sm text-slate-700">
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-slate-100 pt-4">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase text-slate-400 tracking-wider">Store</p>
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-slate-100 pt-4">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase text-slate-400 tracking-wider">Yarn Received</p>
                  <p className="mt-1 font-semibold text-emerald-700">{parseFloat(viewingGp.quantity).toFixed(2)}</p>
                </div>
              </div>

              {viewingGp.yarn_return && (
                <div className="border-t border-slate-100 pt-4">
                  <p className="text-xs font-semibold uppercase text-slate-400 tracking-wider">Yarn Return</p>
                  <p className="mt-1 font-semibold text-red-700">{parseFloat(viewingGp.yarn_return).toFixed(2)}</p>
                </div>
              )}
